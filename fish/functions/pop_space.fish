function pop_space --description "Close all windows in current space and destroy it"
    ...
    if test -z "$CURR_SPACE_INDEX" # Check if the variable is empty
        echo "Error: Failed to get current space index. Yabai might not be running or accessible." >&2
        return 1
    end

    # Now that we know CURR_SPACE_INDEX is not empty, we can proceed with the numeric comparison
    if test "$CURR_SPACE_INDEX" -le 1
        echo "cannot delete first space. exiting..." >&2
        return 1
    end
