...
input=$(cat)
target="${TMUX_PANE:-}"
window=$(tmux display-message -p -t "$target" '#W' 2>/dev/null || echo 'Task')
msg=$(echo "$input" | jq -r '.last_assistant_message // "Task finished"' | head -c 200)
...
if [ -n "$yabai_wid" ] && [ -n "$pane_id" ]; then
    terminal-notifier \
        -title 'ClaudeCode Task finished' \
        -subtitle "$window" \
        -message "$msg" \
        -sound default \
    ...
    terminal-notifier -title 'ClaudeCode Task finished' -subtitle "$window" -message "$msg" -sound default
fi
