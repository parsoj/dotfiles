...
** Dock
- I don't use the Dock, so ~dock_defaults.sh~ effectively disables it
  (auto-hide with a huge reveal delay — macOS won't let you kill the Dock
  process outright since it also runs Mission Control and cmd+tab).
  Run it once on a new machine; undo instructions are in the script.
** Menu Bar
